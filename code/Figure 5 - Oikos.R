...
################
# with color 
Fig3a <- ggplot(data=Fig3data, aes(x=TOTP_avg,y=FPcover_max)) + geom_point() 
Fig3a <- Fig3a + geom_line(aes(x=TOTP_avg,y=fitted),colour="black")
Fig3a <- Fig3a + xlab("Total P (mg/L)") + ylab("Floating plant cover (%)")
Fig3a <- Fig3a + geom_text(aes(x=0.0075,y=1,label="a)"),size=4)
...
Fig3a <- Fig3a + theme_classic(base_size=12)
...
# With color 
Fig3b <- ggplot(dataFP,aes(x=TOTP_avg,y=FPcover_max,shape=factor(beta_logit_cluster_prior_clusterv3))) + stat_smooth(method=glm, family=binomial, se=F,colour="black") + geom_point() 
Fig3b <- Fig3b + scale_shape_manual(values=c(1,19),name="Cluster")
Fig3b <- Fig3b + xlab("Total P (mg/L)") + ylab("Floating plant cover (%)")
Fig3b <- Fig3b + geom_text(aes(x=0.0075,y=1,label="b)"),size=4)
Fig3b <- Fig3b + scale_x_log10()
y_breaks <- seq(0,1,0.25)
y_labels <- as.character(y_breaks*100)
Fig3b <- Fig3b + scale_y_continuous(breaks=y_breaks,labels=y_labels)
Fig3b <- Fig3b + geom_vline(xintercept=0.01972,colour="black",linetype="longdash") # add vertical line @ lower threshold value 
Fig3b <- Fig3b + geom_vline(xintercept=0.2085,colour="black",linetype="longdash") # add vertical line @ upper threshold value 
Fig3b <- Fig3b + theme_classic(base_size=12)
...
ggsave(file="Figure 03.jpg", Fig03, height=16,width=8,units="cm",dpi=1200)
